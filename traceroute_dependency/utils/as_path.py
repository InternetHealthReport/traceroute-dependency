...
class ASPath:
    ...
    def __prune_trailing_timeouts(self) -> None:
        ...
        for idx, (as_, ip) in enumerate(self.nodes[::-1]):
            if isinstance(ip, tuple) and any(x != '*' for x in ip) \
                ...
                break
        if cutoff > 0:
            # Remove all trailing * nodes and replace them with a single * node.
    ...
    def get_reduced_path(self, stats=None) -> (str, str, int):
        ...
        if not self.trailing_timeouts_pruned:
            self.__prune_trailing_timeouts()
        unique_as = set()
        # Used to keep track of how many hops we discarded because we failed to map
        # them. We can use this as an additional filter later, since we might want to
        # ignore paths where we dropped the majority of hops.
        unknown_hops = 0
            ...
            if isinstance(as_, tuple):
                ...
                for sub_idx, entry in enumerate(as_):
                    ...
                    if entry == 'as|0':
                        ...
                        continue
                    # Only remove duplicate AS nodes, i.e., keep IXP nodes, etc.
                ...
                as_path.append(as_)
                ip_path.append(ip)
        # How many percent of the original hops were removed due to failed mapping.
        ...
        if self.__check_start_as(as_path):
            ...
            if self.start_as in unique_as:
                # The specified start AS is somewhere in the path, but not the
                # beginning. Best to ignore.
                if stats:
            ...
            ip_path.insert(0, '*')
            # Shift IXP indexes, since we added a node at the beginning of the path
            # after the index calculation.
        ...
        if self.__check_end_as(as_path):
            ...
            if self.end_as in unique_as:
                # The specified end AS is somewhere in the path, but not the end. Best
                # to ignore.
                if stats:
