...
from metis.shared_extract_functions import (AS_HOPS_FEATURE, IP_HOPS_FEATURE, RTT_FEATURE, VALID_FEATURES, VALID_MODES,
                                            extract_as_hops, extract_ip_hops, extract_rtts, process_window)
...
def main() -> None:
    ...
    window_desc = """Apply a sliding window over the specified time interval.
                     The window length is specified in days with the --window-length
                     parameter, the amount of days to slide the window each iteration
                     with --window-slide-offset."""
    ...
    logging.basicConfig(
        format=FORMAT,
        filename='log/get_candidates.log',
        level=logging.INFO,
        datefmt='%Y-%m-%d %H:%M:%S'
    )
    ...
    if (args.window_length is None and args.window_slide_offset is not None
        ...
        logging.error('Window mode requires both --window-length and --window-slide-offset parameters.')
        sys.exit(1)
    ...
    mode = config.get('options', 'mode')
    enabled_features = config.getcsv('options', 'enabled_features')
    daily_feature_values = defaultdict(lambda: {feature: defaultdict(dict)
                                                for feature in enabled_features})
    ...
    if end_ts != OFFSET_END:
        end_ts //= 1000
    # First, read the entire time interval. If no window is specified, use a placeholder
    # day value.
    ...
    with reader:
        for msg in reader.read():
            ...
            if window_lengths:
                day = int(datetime.fromtimestamp(msg['timestamp'], tz=timezone.utc)
    ...
    first_window_start = datetime.fromtimestamp(start_ts, tz=timezone.utc).replace(hour=0, minute=0, second=0)
    last_window_end = datetime.fromtimestamp(end_ts, tz=timezone.utc).replace(hour=0, minute=0, second=0)
    ...
    if window_lengths:
        for window_length in window_lengths:
            ...
            while window_end <= last_window_end:
                logging.info(f'Processing window {window_start.strftime(DATE_FMT)} -- {window_end.strftime(DATE_FMT)}')
                window_feature_values = process_window(daily_feature_values,
                                                       window_start,
                                                       window_end,
                                                       peer_ids=set())
                curr_output_path = os.path.join(output_path,
                                                window_length,
                ...
                for feature in enabled_features:
                    ...
                    output_file = os.path.join(curr_output_path, f'{input_topic}.{feature}{DATA_SUFFIX}')
        ...
        curr_output_path = os.path.join(output_path,
                                        f'{first_window_start.strftime(DATE_FMT_SHORT)}'
