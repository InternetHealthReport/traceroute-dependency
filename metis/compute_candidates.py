...
def main() -> None:
    ...
    logging.basicConfig(
        format=FORMAT,
        filename='log/compute_candidates.log',
        level=logging.INFO,
        datefmt='%Y-%m-%d %H:%M:%S'
    )
    ...
    with reader:
        for msg in reader.read():
            # Only include values coming from probe ASes and going to non-probe ASes.
    ...
    # We require that a destination was reached from at least half of the probe ASes.
