...
from utils.helper_functions import check_key, check_keys
...
def build_as_path(hops: list, src_as: str) -> list:
    ...
    for hop in hops:
        if check_keys(['hop', 'asn'], hop):
            ...
            return list()
        if (hop['asn'] in included_ases
                or hop['asn'].startswith('-')
                or hop['asn'] == '0'):
            continue
...
def process_as_path(dst_asn: int, as_path: list, hop_counts: dict) -> None:
    ...
    for idx, (ip_hop, asns) in enumerate(as_path):
        if dst_asn in asns:
            ...
            break
    if dst_asn_idx < 0:
        logging.debug(f'AS path did not reach destination AS {dst_asn}: {as_path}')
        return
...
def filter_by_peers(day_values: dict, peer_ids: set) -> None:
    ...
    for feature in day_values:
        day_values[feature] = {peer: {dst: day_values[feature][peer][dst]
                                      for dst in peer_ids
            ...
            if peer_ids:
                for peer, dst in permutations(peer_ids, 2):
                    ...
                    if (curr_val is None and other_val is not None
                            or curr_val is not None and other_val is not None and other_val < curr_val):
                        window_data[feature][peer][dst] = other_val
            else:
                for peer in window_data[feature]:
                    # Check for all destinations from peer that are in the window if
                    # they have a daily value and if that value is smaller.
                    for dst, curr_val in window_data[feature][peer].items():
                        if (dst in daily_values[curr_ts][feature][peer]
                                and daily_values[curr_ts][feature][peer][dst] < curr_val):
                            window_data[feature][peer][dst] = daily_values[curr_ts][feature][peer][dst]
                    # Possible destinations from peer that are not currently in the
                    # window_data.
                    for dst in daily_values[curr_ts][feature][peer].keys() - window_data[feature][peer].keys():
                        window_data[feature][peer][dst] = daily_values[curr_ts][feature][peer][dst]
                # Possible peers that are not currently in the window_data.
                for peer in daily_values[curr_ts][feature].keys() - window_data[feature].keys():
                    window_data[feature][peer] = daily_values[curr_ts][feature][peer]
        curr_day += timedelta(days=1)
    return window_data
